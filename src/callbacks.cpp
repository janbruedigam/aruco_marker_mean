...
void ArucoMarkerMean::aruco_ros_markers_callback(const aruco_msgs::MarkerArray markers)
{
    ...
    marker_mean_pose_.position.x = 0;
    marker_mean_pose_.position.y = 0;
    marker_mean_pose_.position.z = 0;
    ...
    for(int i=0;i<markers.markers.size();i++)
    {
        ...
        marker_mean_pose_.position.x += markers.markers[i].pose.pose.position.x + transform.transform.translation.x;
        marker_mean_pose_.position.y += markers.markers[i].pose.pose.position.y + transform.transform.translation.y;
        marker_mean_pose_.position.z += markers.markers[i].pose.pose.position.z + transform.transform.translation.z; 
    ...
    marker_mean_pose_.position.x /= marker_counter;
    marker_mean_pose_.position.y /= marker_counter;
    marker_mean_pose_.position.z /= marker_counter;

    Eigen::SelfAdjointEigenSolver<Matrix4d> eigen_solver(A);
    Vector4d q_mean = eigen_solver.eigenvectors().col(3); // largest eigenvector

    marker_mean_pose_.orientation.x = q_mean[0];
    marker_mean_pose_.orientation.y = q_mean[1];
    marker_mean_pose_.orientation.z = q_mean[2];
    marker_mean_pose_.orientation.w = q_mean[3];

    marker_mean_pose_pub_.publish(marker_mean_pose_);    
}
